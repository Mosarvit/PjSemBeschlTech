...
# 
# GSI-PBRF (D. Lens), 2016-2018

version_string = 'Rev. 0.2.13, 12.05.2018'

# History
# Rev. 0.2.9: Removed bug in ReadCSV (.type was not always initialized)
# Rev. 0.2.11: Removed import of unnecessary packages
# Rev. 0.2.12: Added functions in ReadCSV class that return the column type, unit, and comment information
# Rev. 0.2.13: Changed import due to new project structure
...
class ReadCSV:
	...
	## Return the unit of a specific column of a CSV file
	#  @param column_number: Integer that specifies the column, where the counting starts at 1 (first column)
	def return_column_unit(self, column_number):
		if not self.flag_read_valid:
			return -1
		if column_number > 0 and column_number <= self.columns:
			return self.column_units[column_number-1]
		else:
			return -1
			
			
	## Return the type of a specific column of a CSV file
	#  @param column_number: Integer that specifies the column, where the counting starts at 1 (first column)
	def return_column_type(self, column_number):
		if not self.flag_read_valid:
			return -1
		if column_number > 0 and column_number <= self.columns:
			return self.column_types[column_number-1]
		else:
			return -1
			
		
	## Return the comment of a specific column of a CSV file
	#  @param column_number: Integer that specifies the column, where the counting starts at 1 (first column)
	def return_column_comment(self, column_number):
		if not self.flag_read_valid:
			return ''
		if column_number > 0 and column_number <= self.columns:
			return self.column_comments[column_number-1]
		else:
			return ''
	...
	# - Remove leading and trailing spaces of an entry (python: string.strip())	
		...
		if self.identifier in ['Ramp File', 'Time Function File']:
			...
			if len(lines) >= 2: # Second line:
				self.column_comments = [temp_string.strip() for temp_string in lines[1].split(",")]
			else:
		...
		# Column file:
			...
			if len(lines) >= 4:
				...
				self.column_comments = [temp_string.strip() for temp_string in lines[3].split(",")]
...
class WriteCSV:
	...
	def write_ramp_file(self,ramp_type,ramp_unit,y_comment_string,t,y,delimiter=',',overwrite=True):
		...
		self.fout.write('Time/s, ' + y_comment_string + '\n')
	...
	# Usage example:
	# write_column_file([6, 103], [6, 3], ['Time/s','Signal/V'], np.ndarray([[0,1],[1,0.5],[2,0]]))
	#  @param self The object pointer
