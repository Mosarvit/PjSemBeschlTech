...
version_string = '0.2.12, 03.01.2018'
...
class ReadBCF:
	...
	def read_header(self):
		...
		if (temp == b'HEAD') == False:
			...
			return -1
		n_bytes = unpack('I',self.fin.read(4))[0]
		self.time_stamp = unpack('d',self.fin.read(8))[0]             # 100 ns steps since epoch
		self.time_stamp_string = datetime.datetime.fromtimestamp(self.time_stamp/1e7).strftime('%Y-%m-%d %H:%M:%S.%f')[0:-3]
		self.number_of_segments = unpack('I',self.fin.read(4))[0]
		self.number_of_columns = unpack('I',self.fin.read(4))[0]
		...
		for col_cnt in range(0,self.number_of_columns):
			...
			if (temp == b'COL ') == False:
				...
				return -1
			n_bytes = unpack('I',self.fin.read(4))[0]
			quantity_type = unpack('I',self.fin.read(4))[0]
			unit = unpack('I',self.fin.read(4))[0]
			representation = unpack('I',self.fin.read(4))[0]
		...
		for seg_cnt in range(0,self.number_of_segments):
			...
			self.segment_times_in_100ns[seg_cnt] = unpack('d',self.fin.read(8))[0]
			self.segment_number_of_rows[seg_cnt] = unpack('I',self.fin.read(4))[0]
...
class WriteBCF:
	...
	def write_header(self):
		...
		self.fout.write(pack('I',n_bytes)) # number of bytes of header content
		# header content
		self.fout.write(pack('d',self.time_stamp)) # time stamp; for current time stamp, use: self.fout.write(pack('d',time.time()*1e7))
		self.fout.write(pack('I',self.number_of_segments)) # number of segments, may be equal to zero if the value is not specified in advance; this value will be updated for each new segment that is added (see function write_segment() of this class.
		self.fout.write(pack('I',self.number_of_columns)) # number of columns per segment
		...
		for col_cnt in range(0,self.number_of_columns):
			...
			self.fout.write(pack('I',n_bytes))
			self.fout.write(pack('I',self.column_information[col_cnt][0])) # write quantity type, 32 bit unsigned
			self.fout.write(pack('I',self.column_information[col_cnt][1])) # write unit, 32 bit unsigned
			self.fout.write(pack('I',self.column_information[col_cnt][2])) # write representation, 32 bit unsigned
			self.fout.write(bytearray([0 for x in range(0,256)])) # 256 bytes of zeros (reserved)
	...
	def write_segment(self,relative_time_in_100ns,x):
		...
		self.fout.write(pack('I',n_bytes)) # number of bytes of content
		# write segment header:
		self.fout.write(pack('d',relative_time_in_100ns)) # relative time with respect to first segment
		self.fout.write(pack('I',number_of_rows)) # number of rows in this segment
		self.fout.write(bytearray([0 for z in range(0,256)])) # 256 bytes of zeros (reserved)
		# write values:
		...
		if flag_all_have_equal_representation:
			...
			for row_cnt in range(0,number_of_rows):
				for col_cnt in range(0,self.number_of_columns):
					if self.column_information[col_cnt][2] == 0: # float representation
						self.fout.write(pack('f',x[row_cnt][col_cnt]))
					if self.column_information[col_cnt][2] == 1: # double representation
						self.fout.write(pack('d',x[row_cnt][col_cnt]))
		...
		if self.flag_update_number_of_segments:
			...
			number_of_segments = unpack('I',self.fout.read(4))[0]
			self.fout.seek(48,0)
			self.fout.write(pack('I',number_of_segments+1)) # increase number of segments by 1
