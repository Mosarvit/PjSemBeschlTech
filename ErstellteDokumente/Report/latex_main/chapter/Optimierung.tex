...
Nutzt man die Optimierung für $\Hcompl$ im nahezu linearen Bereich der Kennlinie $K$, übertragen sich die prinzipiellen Erkenntnisse aus dem \mock-System  auf die Kavität und es lässt sich auch der Einfluss von Rauschen auf die Optimierung feststellen. Wie sich die Kombination aus Anpassung kleiner Beträge in den Spektren der genutzten Ausgangssignale und Einschränkung des Korrekturterms auswirkt, ist in \figref{fig:opt.H.RMS_fabs} aufgetragen. Klar erkennbar sind wieder die Problematik der Nulldurchgänge und die Auflösung hoher Frequenzen.
Einen Überblick über die Variation der Parameter der Optimierung bietet \figref{subfig:opt.H.qualityOverview}. 
...
\end{axis}
\end{tikzpicture}
\caption{Korrekturterm in initialer und in angepasster Form mit RMS- und $3 \promille$-Korrektur, am Messaufbau}
...
\begin{tikzpicture}
	...
	\addplot [blue, mark = x] table [col sep = comma, x expr=\coordindex+1, y index =1]{			
		...
		}; \addlegendentry{Ohne Anpassungen Messung 1}	
	\addplot [blue, mark = +] table [col sep = comma, x expr=\coordindex+1, y index =1]{			
		...
		}; \addlegendentry{Ohne Anpassungen Messung 2}	
...
\end{axis}
%\end{tikzpicture}
%	\caption{Einfluss unterschiedlicher Parameterwahl auf Entwicklung der Übertragungsfunktion bei niedrigen Frequenzen}
%	\label{subfig:opt.H.lowFreqParam}
%	\end{subfigure}
%	
%	
%	\begin{subfigure}{0.5 \textwidth}
...
\end{axis}
\end{tikzpicture}
	\caption{Einfluss unterschiedlicher Parameterwahl auf Entwicklung der Übertragungsfunktion bei hohen Frequenzen}
	\end{subfigure}
...
\begin{subfigure}{0.5 \textwidth}
	...
	\importTikz{adjust_a_K_bearbeitet.tikz}	
...
\begin{subfigure}{0.5 \textwidth}
	\setlength\figureheight{8cm}
	\setlength\figurewidth{8cm}
    %\input{images/plots/adjust_a_Uin.tikz}
    \importTikz{adjust_a_Uout.tikz}
...
Das eingezeichnete Maximum von $\Uquest$ in \figref{fig:K0} liegt über der zulässigen Grenze von $K_0$, deshalb kann damit kein vorverzerrtes Signal berechnet werden.
